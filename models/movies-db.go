...
func (m *DBModel) InsertMovie(movie Movie) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt, movie.Title, movie.Description, movie.Year, movie.ReleaseDate, movie.Runtime, movie.Rating, movie.MPAARating, movie.CreatedAt, movie.UpdatedAt)
	if err != nil {
		return err
	}
...
func (m *DBModel) UpdateMovie(movie Movie) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt, movie.Title, movie.Description, movie.Year, movie.ReleaseDate, movie.Runtime, movie.Rating, movie.MPAARating, movie.UpdatedAt, movie.ID)
	if err != nil {
		return err
	}
