...
func (m *DBModel) Get(id int) (*Movie, error) {
	...
	err := row.Scan(&movie.ID, &movie.Title, &movie.Description, &movie.Year, &movie.ReleaseDate, &movie.Rating, &movie.Runtime, &movie.MPAARating, &movie.CreatedAt, &movie.UpdatedAt, &movie.Poster)
	if err != nil {
		return nil, err
	}
...
func (m *DBModel) All(genre ...int) ([]*Movie, error) {
	...
	for rows.Next() {
		var movie Movie
		err := rows.Scan(&movie.ID, &movie.Title, &movie.Description, &movie.Year, &movie.ReleaseDate, &movie.Rating, &movie.Runtime, &movie.MPAARating, &movie.CreatedAt, &movie.UpdatedAt, &movie.Poster)
		if err != nil {
			return nil, err
		}
...
func (m *DBModel) GenresAll() ([]*Genre, error) {
	...
	for rows.Next() {
		var g Genre
		err := rows.Scan(&g.ID, &g.GenreName, &g.CreatedAt, &g.UpdatedAt)
		if err != nil {
			return nil, err
		}
...
func (m *DBModel) GenreOne(id int) (*Genre, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	...
	err := row.Scan(&genre.ID, &genre.GenreName, &genre.CreatedAt, &genre.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (m *DBModel) InsertMovie(movie Movie) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt, movie.Title, movie.Description, movie.Year, movie.ReleaseDate, movie.Runtime, movie.Rating, movie.MPAARating, movie.CreatedAt, movie.UpdatedAt, movie.Poster)
	if err != nil {
		return err
	}
...
func (m *DBModel) UpdateMovie(movie Movie) error {
	...
	_, err := m.DB.ExecContext(ctx, stmt, movie.Title, movie.Description, movie.Year, movie.ReleaseDate, movie.Runtime, movie.Rating, movie.MPAARating, movie.UpdatedAt, movie.Poster, movie.ID)
	if err != nil {
		return err
	}
